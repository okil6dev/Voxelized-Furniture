...
public class YuccaPlantSettingsGuiScreen extends AbstractContainerScreen<YuccaPlantSettingsGuiMenu> implements VoxelizedFurnitureModScreens.FabricScreenAccessor {
	...
	public void init() {
		...
		button_normal_2d = Button.builder(Component.translatable("gui.voxelized_furniture.yucca_plant_settings_gui.button_normal_2d"), e -> {
			...
			if (true) {
				...
				ClientPlayNetworking.send(new YuccaPlantSettingsGuiButtonMessage(0, x, y, z));
			}
		...
		button_random_2d = Button.builder(Component.translatable("gui.voxelized_furniture.yucca_plant_settings_gui.button_random_2d"), e -> {
			...
			if (true) {
				...
				// The packet handler runs the procedure server-side; executing it here as well
				// causes client/server inventory desynchronization for procedures that modify slots.
				ClientPlayNetworking.send(new YuccaPlantSettingsGuiButtonMessage(1, x, y, z));
			}
