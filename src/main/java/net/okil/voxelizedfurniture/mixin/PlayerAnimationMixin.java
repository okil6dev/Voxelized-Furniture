...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		VoxelizedFurnitureModPlayerAnimationAPI.PlayerAnimation animation = VoxelizedFurnitureModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (!master.equals("voxelized_furniture")) {
			if (!VoxelizedFurnitureModPlayerAnimationAPI.animations.isEmpty())
				VoxelizedFurnitureModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			VoxelizedFurnitureModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		VoxelizedFurnitureModPlayerAnimationAPI.PlayerAnimation animation = VoxelizedFurnitureModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = VoxelizedFurnitureModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				VoxelizedFurnitureMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			VoxelizedFurnitureModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, VoxelizedFurnitureModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			VoxelizedFurnitureModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = VoxelizedFurnitureModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = VoxelizedFurnitureModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = VoxelizedFurnitureModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
