...
public class CookingGuiMenu extends AbstractContainerMenu implements VoxelizedFurnitureModMenus.MenuAccessor {
	...
	public CookingGuiMenu(int id, Inventory inv) {
	...
	public CookingGuiMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		...
		if (extraData != null && !inv.player.level().isClientSide()) {
			...
			try {
				...
				if (extraData.readableBytes() == 0) {
					...
					if (blockEntity instanceof Container blockContainer && blockContainer.getContainerSize() >= 3) {
	...
	public CookingGuiMenu(int id, Inventory inv, Container container, FriendlyByteBuf extraData) {
	...
	public CookingGuiMenu(int id, Inventory inv, Container container) {
		super(VoxelizedFurnitureModMenus.COOKING_GUI, id);
		this.entity = inv.player;
		this.world = inv.player.level();
		this.inventory = container;
		this.customSlots.put(0, this.addSlot(new Slot(inventory, 0, 46, 33) {
			private final int slot = 0;
			private int x = CookingGuiMenu.this.x;
			private int y = CookingGuiMenu.this.y;
		}));
		this.customSlots.put(1, this.addSlot(new Slot(inventory, 1, 46, 69) {
			private final int slot = 1;
			private int x = CookingGuiMenu.this.x;
			private int y = CookingGuiMenu.this.y;
		}));
		this.customSlots.put(2, this.addSlot(new Slot(inventory, 2, 118, 51) {
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 2 + 8 + sj * 18, 22 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 2 + 8 + si * 18, 22 + 142));
	}
