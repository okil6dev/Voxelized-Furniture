...
public class ShelfGuiMenu extends AbstractContainerMenu implements VoxelizedFurnitureModMenus.MenuAccessor {
	...
	public ShelfGuiMenu(int id, Inventory inv) {
	...
	public ShelfGuiMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		...
		if (extraData != null && !inv.player.level().isClientSide()) {
			...
			try {
				...
				if (extraData.readableBytes() == 0) {
					...
					if (blockEntity instanceof Container blockContainer && blockContainer.getContainerSize() >= 4) {
	...
	public ShelfGuiMenu(int id, Inventory inv, Container container, FriendlyByteBuf extraData) {
	...
	public ShelfGuiMenu(int id, Inventory inv, Container container) {
		super(VoxelizedFurnitureModMenus.SHELF_GUI, id);
		this.entity = inv.player;
		this.world = inv.player.level();
		this.inventory = container;
		this.customSlots.put(0, this.addSlot(new Slot(inventory, 0, 49, 20) {
			private final int slot = 0;
			private int x = ShelfGuiMenu.this.x;
			private int y = ShelfGuiMenu.this.y;
		}));
		this.customSlots.put(1, this.addSlot(new Slot(inventory, 1, 67, 20) {
			private final int slot = 1;
			private int x = ShelfGuiMenu.this.x;
			private int y = ShelfGuiMenu.this.y;
		}));
		this.customSlots.put(2, this.addSlot(new Slot(inventory, 2, 85, 20) {
			private final int slot = 2;
			private int x = ShelfGuiMenu.this.x;
			private int y = ShelfGuiMenu.this.y;
		}));
		this.customSlots.put(3, this.addSlot(new Slot(inventory, 3, 103, 20) {
