...
public class WardrobeGuiMenu extends AbstractContainerMenu implements VoxelizedFurnitureModMenus.MenuAccessor {
	...
	public WardrobeGuiMenu(int id, Inventory inv) {
		...
		if (extraData != null && !inv.player.level().isClientSide()) {
			...
			try {
				...
				if (extraData.readableBytes() == 0) {
					...
					if (blockEntity instanceof Container blockContainer && blockContainer.getContainerSize() >= 8) {
	...
	public WardrobeGuiMenu(int id, Inventory inv, Container container, FriendlyByteBuf extraData) {
	...
	public WardrobeGuiMenu(int id, Inventory inv, Container container) {
		super(VoxelizedFurnitureModMenus.WARDROBE_GUI, id);
		this.entity = inv.player;
		this.world = inv.player.level();
		this.inventory = container;
		this.customSlots.put(0, this.addSlot(new Slot(inventory, 0, 42, 12) {
			private final int slot = 0;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(1, this.addSlot(new Slot(inventory, 1, 60, 12) {
			private final int slot = 1;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(2, this.addSlot(new Slot(inventory, 2, 42, 30) {
			private final int slot = 2;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(3, this.addSlot(new Slot(inventory, 3, 60, 30) {
			private final int slot = 3;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(4, this.addSlot(new Slot(inventory, 4, 87, 12) {
			private final int slot = 4;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(5, this.addSlot(new Slot(inventory, 5, 105, 12) {
			private final int slot = 5;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(6, this.addSlot(new Slot(inventory, 6, 87, 30) {
			private final int slot = 6;
			private int x = WardrobeGuiMenu.this.x;
			private int y = WardrobeGuiMenu.this.y;
		}));
		this.customSlots.put(7, this.addSlot(new Slot(inventory, 7, 105, 30) {
